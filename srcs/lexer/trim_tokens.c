...
/*   Updated: 2024/11/07 08:41:03 by marlonco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    trim_tokens(t_list *tokens)
{
    ...
    while (tokens)
    {
        if (((char *)tokens->value[0] == '\"' && (char *)tokens->value[ft_strlen(tokens->value) - 1] == '\"')
                || ((char *)tokens->value[0] == '\'' && (char *)tokens->value[ft_strlen(tokens->value) - 1] == '\''))
            remove_quotes(tokens);
        i = 0;
        while (&tokens->value[i])
        {
            if ((char *)tokens->value[i] == '$' && &tokens->value[i + 1])
                refer_envv(tokens, i);
            i++;
        }
        tokens = tokens->next;
